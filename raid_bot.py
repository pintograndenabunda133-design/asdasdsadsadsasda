...
async def audit_restore(guild, log_channel=None):
    ...
    1. Lê TODO o audit log (sem limite de tempo)
    2. Debuga o tipo do target pra entender o que o Discord retorna
    3. Identifica o raider pelo padrão de deleções em bloco
    4. Apaga canais de raid e recria originais
    """
    import time
    ...
    # === DEBUG: Verificar o que o target realmente é ===
    await log("\n🔎 **Debugando targets do Audit Log:**")
    sample_targets = []
    for entry in channel_deletions[:5]:
        target = entry.target
        target_type = type(target).__name__
        target_repr = repr(target)[:100]
        # Tentar pegar o nome de várias formas
        target_name = "N/A"
        try:
            target_name = str(target.name) if hasattr(target, 'name') else "no_name"
        except Exception:
            pass
        sample_targets.append(f"  • Tipo: `{target_type}` | Nome: `{target_name}` | Repr: `{target_repr}`")
    ...
    # === COLETAR CANAIS ORIGINAIS ===
    # O target pode ser:
    # - discord.PartialInviteGuild (objeto genérico) - NÃO tem .name direto
    # - discord.TextChannel/VoiceChannel/CategoryChannel - tem .name, .category
    # - discord.Object - objeto genérico sem atributos
    ...
    for entry in channel_deletions:
        ...
        # Tentativa 1: target é um canal real (tem .name)
        ...
        # Tentativa 2: target é um Object com ID mas sem nome
        elif hasattr(target, 'id'):
            # Usar o change do audit log pra tentar pegar o nome
            ch_name = f"canal-{target.id}"
            ch_position = 0
            cat_id = None
            cat_name = None
            
            # Tentar pegar info extra do entry.change
            try:
                if hasattr(entry, 'changes'):
                    for change in entry.changes:
                        if change.key == 'name':
                            ch_name = change.after if change.after else ch_name
            except Exception:
                pass
            
            skipped_unknown += 1
            # Pular canais sem nome válido
            if ch_name.startswith("canal-"):
                continue
        else:
            skipped_unknown += 1
            continue
    ...
    await log(f"\n📁 **Canais originais encontrados:** {len(original_channels)}")
    if skipped_unknown > 0:
        await log(f"⚠️ **Canais ignorados (sem nome no log):** {skipped_unknown}")
    ...
    if not original_channels:
        return False, "❌ **Não encontrei canais originais com nome válido no Audit Log.** Tente `!restore` se o bot já esteve aqui antes, ou use `!shop setup` pra criar do zero."
    
    # Mostrar alguns canais originais encontrados
    await log("\n📋 **Canais originais que vou recriar:**")
    for ch in original_channels[:15]:
        await log(f"  • {ch['name']} (cat: {ch['category_name'] or 'sem categoria'})")
    ...
    msg = (
        ...
        f"📋 **Relatório Final:**\n"
        f"• Canais originais encontrados: **{len(original_channels)}**\n"
        f"• Canais ignorados (sem nome): **{skipped_unknown}**\n"
...
async def audit_restore(guild, log_channel=None):
    ...
    Lê o Audit Log pra descobrir quais canais foram apagados recentemente
    e tenta reconstruir o estado original do servidor.
    log_channel: canal pra enviar mensagens de progresso
    """
    import time
    ...
    # Filtrar canais apagados nos últimos 24h que provavelmente são do servidor original
    now = time.time()
    original_channels = []
    for entry in channel_deletions:
    ...
    if not original_channels:
        return False, "❌ **Não encontrei canais originais no Audit Log.** O log pode ter expirado (Discord mantém apenas 90 dias)."
    
    # Mostrar alguns canais originais encontrados
    await log("\n📋 **Canais originais que vou recriar:**")
    for ch in original_channels[:10]:  # Mostrar apenas os primeiros 10
        await log(f"  • {ch['name']} (cat: {ch['category_name'] or 'sem categoria'})")
    if len(original_channels) > 10:
        await log(f"  ... e mais {len(original_channels) - 10} canais")
